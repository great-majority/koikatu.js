...
function readPatternInfo(reader: BinaryReader): Record<string, any> {
  return {
    ...
    filepath: required(
      ...
      'Failed to read pattern.filepath',
    ),
    clamp: required(reader.readUint8(), 'Failed to read pattern.clamp') !== 0,
    uv: JSON.parse(
      required(reader.read7BitEncodedString(), 'Failed to read pattern.uv'),
    ),
...
function writePatternInfo(
  ...
  writer.write7BitEncodedString(pattern.filepath ?? '');
  writer.writeUint8(pattern.clamp === false ? 0 : 1);
  writer.write7BitEncodedString(
    JSON.stringify(pattern.uv ?? { x: 0, y: 0, z: 1, w: 1 }),
  );
...
function readRoutePointInfo(reader: BinaryReader): Record<string, any> {
  return {
    ...
    aidInfo: readRoutePointAidInfo(reader),
    link: required(reader.readUint8(), 'Failed to read routePoint.link') !== 0,
  };
}
...
function loadCharInfo(
  ...
  if (embedded.pngBytes) {
    data.characterPng = embedded.pngBytes;
  }
  ...
  for (let index = 0; index < ikCount; index += 1) {
    ...
    data.ik_targets[key] = readBoneInfo(reader, `char.ikTargets.${key}`);
  }
  ...
  data.anime_info = {
    title: required(
      reader.readInt32LE(),
      'Failed to read char.anime_info.title',
    ),
    group: required(
      reader.readInt32LE(),
  ...
  data.unknown = required(reader.readBytes(14), 'Failed to read char.unknown');
  data.enable_ik =
  ...
  data.expression = readBoolArray(reader, 9, 'char.expression');
  ...
  const voiceLength = required(
    reader.readInt32LE(),
    'Failed to read char.voiceCtrl length',
  );
  const voiceReader = new BinaryReader(
    required(
      reader.readBytes(voiceLength),
      'Failed to read char.voiceCtrl bytes',
    ),
  );
  data.voiceCtrl = {
    unknown: required(
      voiceReader.readUint8(),
      'Failed to read char.voiceCtrl.unknown',
    ),
    list: [] as Array<Record<string, number>>,
  };
  const voiceCount = required(
    voiceReader.readInt32LE(),
    'Failed to read char.voiceCtrl.count',
  );
  ...
  data.anime_option_param = [
    ...
    required(
      reader.readFloat32LE(),
      'Failed to read char.anime_option_param[2]',
    ),
  ];

  const neckLength = required(
    reader.readInt32LE(),
    'Failed to read char.neck length',
  );
  ...
  const eyesLength = required(
    reader.readInt32LE(),
    'Failed to read char.eyes length',
  );
...
function saveCharInfo(
  ...
  const bones = Object.entries(data.bones ?? {}) as [
    string,
    Record<string, any>,
  ][];
  ...
  const ikTargets = Object.entries(data.ik_targets ?? {}) as [
    string,
    Record<string, any>,
  ][];
  ...
  writer.writeInt32LE(data.anime_info?.title ?? 0);
  ...
  writer.writeBytes(data.unknown ?? new Uint8Array(14));
  ...
  writeBoolArray(
    writer,
    Array.from({ length: 9 }, (_, index) => Boolean(data.expression?.[index])),
  );
  ...
  const voiceWriter = new BinaryWriter();
  voiceWriter.writeUint8(data.voiceCtrl?.unknown ?? 2);
  const voices = data.voiceCtrl?.list ?? [];
  voiceWriter.writeInt32LE(voices.length);
  for (const voice of voices) {
    voiceWriter.writeInt32LE(voice.group ?? 0);
    voiceWriter.writeInt32LE(voice.category ?? 0);
    voiceWriter.writeInt32LE(voice.no ?? 0);
  }
  voiceWriter.writeInt32LE(data.voiceCtrl?.repeat ?? 0);
  const voiceBytes = voiceWriter.toUint8Array();
  writer.writeInt32LE(voiceBytes.length);
  writer.writeBytes(voiceBytes);
  ...
  const optionParam = data.anime_option_param ?? [0, 0, 0];
  writer.writeFloat32LE(optionParam[0] ?? 0);
  writer.writeFloat32LE(optionParam[1] ?? 0);
  writer.writeFloat32LE(optionParam[2] ?? 0);
...
function loadItemInfo(
  ...
  data.title = required(reader.readInt32LE(), 'Failed to read item.title');
  ...
  data.shadow_type = required(
    reader.readInt32LE(),
    'Failed to read item.shadow_type',
  );
  data.shadow_switch =
    required(reader.readUint8(), 'Failed to read item.shadow_switch') !== 0;
  data.shadow_strength = required(
    reader.readFloat32LE(),
    'Failed to read item.shadow_strength',
  );
  data.patterns = [
  ...
  data.line_color = JSON.parse(
    required(reader.read7BitEncodedString(), 'Failed to read item.line_color'),
  );
  data.line_width = required(
    reader.readFloat32LE(),
    'Failed to read item.line_width',
  );
  data.emission_color = JSON.parse(
    required(
      reader.read7BitEncodedString(),
  ...
  data.enable_dynamic_bone =
    required(reader.readUint8(), 'Failed to read item.enable_dynamic_bone') !==
    0;
  data.anime_normalized_time = required(
    reader.readFloat32LE(),
...
function saveItemInfo(
  ...
  const data = object.data;
  writeObjectInfoBase(writer, data);
  writer.writeInt32LE(data.title ?? 0);
  ...
  writer.writeInt32LE(data.shadow_type ?? 0);
  writer.writeUint8(data.shadow_switch ? 1 : 0);
  writer.writeFloat32LE(data.shadow_strength ?? 0);
  ...
  writer.writeFloat32LE(data.line_width ?? 0);
...
function loadLightInfo(reader: BinaryReader): SceneObject {
  ...
  data.lightSwitch =
    required(reader.readUint8(), 'Failed to read light.lightSwitch') !== 0;
  data.displayTarget =
    required(reader.readUint8(), 'Failed to read light.displayTarget') !== 0;
  data.color = JSON.parse(
    required(reader.read7BitEncodedString(), 'Failed to read light.color'),
  );
  ...
  data.outsideSpotAngle = required(
    reader.readFloat32LE(),
    'Failed to read light.outsideSpotAngle',
  );
  data.insideSpotAngle = required(
    reader.readFloat32LE(),
    'Failed to read light.insideSpotAngle',
  );
  data.shadow =
    required(reader.readUint8(), 'Failed to read light.shadow') !== 0;
  data.shadowStrength = required(
    reader.readFloat32LE(),
    'Failed to read light.shadowStrength',
  );
  return { type: 2, data };
}
...
function saveLightInfo(writer: BinaryWriter, object: SceneObject): void {
  ...
  writeObjectInfoBase(writer, data);
  writer.writeInt32LE(data.no ?? 0);
  writer.writeUint8(data.lightSwitch ? 1 : 0);
  writer.writeUint8(data.displayTarget ? 1 : 0);
  ...
  writer.writeFloat32LE(data.outsideSpotAngle ?? 0);
  writer.writeFloat32LE(data.insideSpotAngle ?? 0);
  writer.writeUint8(data.shadow ? 1 : 0);
  writer.writeFloat32LE(data.shadowStrength ?? 0);
}
...
function loadRouteInfo(
  ...
  for (let index = 0; index < count; index += 1) {
    data.route_points.push(readRoutePointInfo(reader));
  }
...
function saveRouteInfo(
  ...
  for (const routePoint of routePoints) {
    writeRoutePointInfo(writer, routePoint);
  }
...
export function readHcSceneObject(
  reader: BinaryReader,
  type: number,
  version: string,
  ...
  switch (type) {
    ...
    case 5:
      ...
      break;
    default:
      throw new Error(`Unknown HC scene object type: ${type}`);
  }
...
export function writeHcSceneObject(
  writer: BinaryWriter,
  object: SceneObject,
  version: string,
): void {
  ...
  switch (object.type) {
    ...
    case 5:
      ...
      return;
    default:
      throw new Error(`Unknown HC scene object type: ${object.type}`);
  }
}
